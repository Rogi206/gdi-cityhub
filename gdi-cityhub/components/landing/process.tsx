          ...
          {steps.map((step, index) => (
            <div key={index} className="relative">
              ...
              {index < steps.length - 1 && (
                <div className="hidden md:block absolute top-10 left-[60%] w-[80%] h-px bg-gradient-to-r from-[#3B82F6]/50 to-transparent" />
              )}
              
              <div className="relative flex flex-col items-center text-center p-4">
                {/* Step circle with glow */}
                <div className="relative mb-5">
                  <div className="absolute inset-0 bg-[#3B82F6]/30 rounded-full blur-xl" />
                  <div className="relative w-20 h-20 rounded-full bg-gradient-to-br from-[#3B82F6] to-[#2563EB] flex items-center justify-center shadow-lg shadow-[#3B82F6]/30">
                    <step.icon className="w-8 h-8 text-white" />
                  </div>
                </div>
                
                <span className="text-xs font-mono text-[#06B6D4] mb-2 tracking-widest">{step.number}</span>
